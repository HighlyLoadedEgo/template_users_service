...
class UserService(Service):
    ...
    async def check_valid_user(self, user_credentials: LoginUserSchema) -> TokensData:
        return await use_cases.CheckValidUserUseCase(
            uow=self._uow, jwt_manager=self._jwt_manager, msg_broker=self._msg_broker
    ...
    async def create_user(self, create_user_data: CreateUserSchema) -> None:
        return await use_cases.CreateUserUseCase(
            uow=self._uow, msg_broker=self._msg_broker
        )(create_user_data=create_user_data)

    async def update_user(self, update_user_data: UpdateUserSchema) -> None:
        await use_cases.UpdateUserUseCase(uow=self._uow, msg_broker=self._msg_broker)(
            update_user_data=update_user_data
        )

    async def delete_user(self, user_id: UUID) -> None:
        await use_cases.DeleteUserUseCase(uow=self._uow, msg_broker=self._msg_broker)(
            user_id=user_id
        )
